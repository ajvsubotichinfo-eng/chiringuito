...
**Estado general:** ✅ Fases 1 a 4 completas y **funcionando en producción** (base de datos, deploy, backend y frontend con las 4 pantallas + PWA, identidad visual definitiva). Arrancando Fase 5 (usuarios y datos reales)
...
| ingresos | Cierre de caja: fecha, monto, medio de cobro, usuario. Sin proveedor ni comprobante (no es un pago a un tercero) |
...
| 💵 Ingresos | Cierre de caja: registrar lo recaudado por día en uno o varios medios de cobro (efectivo, transferencia, tarjeta/POS, Mercado Pago, otro), con resumen mensual agrupado por medio |
...
### 🌱 Iniciativa paralela — arquitectura multi-tenant (SaaS)
Visión a más largo plazo: convertir este proyecto en un producto vendible a otros
comercios, no solo Carmen T. Documentación completa en
`docs/arquitectura-multi-tenant/` (empezar por `00-INDICE.md`). No reemplaza este
plan ni cambia el uso diario de la tienda — es una capa que se va agregando en
paralelo. Estado actual: **Fase 1 del módulo 01 completa** (2026-09-05: tabla
`tenants` + `tenant_id` en todas las tablas existentes, con Carmen T como único
tenant real) — ver `docs/arquitectura-multi-tenant/01-SPEC-multi-tenant-fase1.md`
para el detalle ejecutado y la bitácora de arriba para el resumen. Pendiente para
la Fase 2: capa que resuelva el tenant desde el usuario logueado (hoy es una
constante fija, `TENANT_ID_ACTUAL` en `src/config/tenant.js`) — recién hace falta
cuando se sume un segundo tenant real.
...
- [x] Agregar una sección donde se registren los ingresos diarios — ✅ hecho el 05/09/2026, ver bitácora
...
| 2026-09-05 | Nueva funcionalidad (del backlog, ajustada en la conversación con el dueño): pantalla de Ingresos, cierre de caja con varios medios de cobro por día (efectivo, transferencia, tarjeta/POS, Mercado Pago, otro) — mismo patrón de "varias líneas" que Pagos, pero sin proveedor ni foto. Tabla nueva `ingresos` en `crm_schema.sql` y creada en la base real. Backend: `GET/POST /api/ingresos`, `PUT /api/ingresos/:id`, y `GET /api/reportes/ingresos-por-mes` (agrupado por medio). Frontend: `Ingresos.jsx` (pestañas Recientes/Por mes, igual que Pagos), `IngresosPorMes.jsx`, `IngresoFormulario.jsx` (líneas repetibles de medio+monto con total). Se agregó un **5to ícono** a la navegación (antes eran 4) — se ajustó el espaciado de la barra flotante en celular para que entraran cómodos. De paso se movieron `MESES`/`mesActual()`/`nombreMes()` de `PagosPorMes.jsx` a `utils.js` para reusarlos sin duplicar código. Probado: registrar un ingreso con dos medios (efectivo + tarjeta/POS) crea dos filas separadas, el resumen mensual agrupa y suma bien, se ve correcto en celular y en escritorio (panel lateral con 5 accesos) — sin errores de consola. |
| 2026-09-05 | **Nueva iniciativa paralela:** se decidió empezar a preparar el proyecto para eventualmente venderse como producto SaaS multi-tenant a otros comercios (no solo Carmen T). Se documentó la arquitectura completa de esa visión en `docs/arquitectura-multi-tenant/` (módulos 00 a 14: multi-tenant, catálogo, stock, TPV, clientes, proveedores, facturación por conectores, impuestos, medios de pago, usuarios/permisos, reportes, planes SaaS, cumplimiento legal, infraestructura). De esa visión, arrancó en concreto la **Fase 1 del módulo 01** (`01-SPEC-multi-tenant-fase1.md`): agregar una tabla `tenants` y una columna `tenant_id` (INT UNSIGNED) a las tablas existentes (`usuarios`, `productos`, `proveedores`, `precios_proveedor`, `historial_precios`, `pagos`, `ingresos`, `configuracion` — esta última necesita además pasar a PK compuesta `(tenant_id, clave)`), con Carmen T como único tenant real. Explícitamente fuera de esta fase: middleware que inyecte `tenant_id` en cada query (queda para antes de sumar un segundo tenant real). Esto no cambia nada de cara al uso diario de la tienda; conviven ambos documentos: este plan sigue siendo la fuente de verdad operativa, `docs/arquitectura-multi-tenant/00-INDICE.md` es la fuente de verdad de esa visión a futuro. |
| 2026-09-05 | **Fase 1 del módulo multi-tenant completada** sobre la base real. Se corrió una migración (script Node temporal, ya borrado — mismo patrón usado para importar `crm_schema.sql` en la Fase 1.2) que: creó la tabla `tenants` con el único tenant real (`Frutos Secos Carmen T`, país AR, moneda ARS — tomada de la que ya tenía configurada la app), agregó `tenant_id` (NOT NULL + FK a `tenants.id`) a `usuarios`, `productos`, `proveedores`, `precios_proveedor`, `historial_precios`, `pagos` e `ingresos`, y en `configuracion` además cambió la clave primaria a `(tenant_id, clave)`. Se verificó que ninguna fila quedó sin `tenant_id`. Se actualizaron los 6 archivos de rutas que insertan datos (`productos.js`, `proveedores.js`, `precios.js`, `pagos.js`, `ingresos.js`, `configuracion.js`) para mandar explícitamente el `tenant_id` (constante `TENANT_ID_ACTUAL` en `src/config/tenant.js`, sin capa de inyección automática todavía — eso es Fase 2). Probado después de migrar: el servidor arranca y `/api/db-test` conecta bien contra la base real. `docs/crm_schema.sql` quedó actualizado como referencia de instalación limpia con este esquema. **Confirmado por el dueño: probó crear producto, proveedor, precio, pago e ingreso desde la app real y todo funciona igual que antes de la migración.** Fase 1 del módulo multi-tenant cerrada de punta a punta. |
| 2026-09-05 | **Bug reportado por el dueño:** creó sin querer un segundo "Agua mineral 500ml" (no hay restricción de nombre único, a propósito, por si dos productos distintos comparten nombre); el buscador del Comparador sí traía los dos, pero al mostrar solo el nombre —sin nada más— eran indistinguibles y parecía que faltaba uno. El dueño pidió ir más a fondo: que crear un producto sin asignarle proveedor y precio no sea posible, y que antes de crear uno nuevo se muestre primero la lista de los que ya existen. Cambios: `POST /api/productos` ahora EXIGE `proveedor_id` y `precio_compra`, e inserta el producto y su primer precio en una sola transacción (si el precio falla, no queda un producto vacío dando vueltas). `ProductoFormulario.jsx` (solo al crear) pasa a tener dos pasos: 1) buscar entre los productos existentes (lista por defecto + filtro en vivo, igual que la pantalla Productos), con un botón "No está en la lista, crear producto nuevo"; 2) recién ahí el formulario, que ahora pide también proveedor + precio de compra (obligatorios). Editar un producto existente no cambió. Se borró el producto duplicado vacío que había quedado de antes. Probado: buscar antes de crear, crear sin precio (el navegador lo bloquea), crear completo (producto + precio quedan creados juntos, sin pasos sueltos) — sin errores de consola. |
